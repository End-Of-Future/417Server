...
function cm.spop(e,tp,eg,ep,ev,re,r,rp)
    ...
    and tc:IsRelateToEffect(e) and Duel.Equip(tp,tc,c) then
        ...
        if Duel.IsExistingMatchingCard(Card.IsFaceup,tp,0,LOCATION_MZONE,1,nil) and Duel.SelectYesNo(tp,16*m) then
            Duel.BreakEffect()
            Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
            local dc=Duel.SelectMatchingCard(tp,Card.IsFaceup,tp,0,LOCATION_MZONE,1,1,nil)
            if dc then
