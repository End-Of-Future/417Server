...
function cm.chkfilter(c,tp)
	return c:IsControler(tp) and c:IsSetCard(0x7d8)
end
...
function cm.fcheck(tp,sg,fc)
	if sg:IsExists(cm.chkfilter,1,nil,tp) then
		...
		return sg:IsExists(Card.IsRace,1,nil,RACE_ZOMBIE) and not sg:IsExists(cm.exfilter,1,nil,tp)
	end
end
function cm.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		if mg1:IsExists(cm.chkfilter,1,nil,tp) and mg2:GetCount()>0 or mg2:IsExists(cm.chkfilter,1,nil,tp) then
			mg1:Merge(mg2)
		end
...
function cm.spfilter1(c,e,tp)
	return c:IsCanBeSpecialSummoned(e,0,tp,false,false) and c:IsCode(14010268)
end
function cm.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(cm.spfilter1,tp,LOCATION_DECK,0,1,nil,e,tp) and Duel.GetMZoneCount(tp)>0 end
