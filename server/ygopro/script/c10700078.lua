...
function cm.initial_effect(c)
    ...
    cm.AddLinkProcedure(c,0,aux.FilterBoolFunction(Card.IsRace,RACE_SPELLCASTER),4,5,cm.lcheck)
    cm.AddLinkProcedure(c,1,nil,3,5,cm.lcheck2)
    c:EnableReviveLimit()
    --splimit
...
function cm.thcost(e,tp,eg,ep,ev,re,r,rp,chk)
    ...
    local g=Duel.GetMatchingGroup(cm.tgsfilter,tp,LOCATION_HAND,0,nil,e,tp,zone)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsDiscardable,tp,LOCATION_HAND,0,1,g) end
	Duel.DiscardHand(tp,Card.IsDiscardable,1,1,REASON_COST+REASON_DISCARD,g)
end
...
function cm.costrfilter(c)
    return c:IsType(0xe0000) and c:IsType(TYPE_SPELL) and c:IsDiscardable()
end
...
function cm.reop(e,tp,eg,ep,ev,re,r,rp)
    ...
    local ops={}
    local opval={}
    if Duel.GetFieldGroupCount(tp,0x04,0)>0 then
