...
function cm.concfilter(c)
	return c:IsFaceup() and not c:IsType(TYPE_SPELL)
end

function cm.condition(e,tp,eg,ep,ev,re,r,rp)
	return (Duel.GetFieldGroupCount(tp,LOCATION_ONFIELD,0)==0 or not Duel.IsExistingMatchingCard(cm.concfilter,tp,LOCATION_ONFIELD,0,1,nil)) and (Duel.GetFieldGroupCount(tp,0,LOCATION_HAND)>=Duel.GetFieldGroupCount(tp,LOCATION_HAND,0)) and (Duel.GetFieldGroupCount(tp,0,LOCATION_ONFIELD)>Duel.GetFieldGroupCount(tp,LOCATION_ONFIELD,0)+1)
end

function cm.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	e:GetHandler():RegisterFlagEffect(m,RESET_EVENT+RESETS_STANDARD-RESET_TOFIELD+RESET_MSCHANGE,0,1)
	Duel.SetChainLimit(cm.chainlm)
end

function cm.spcon1(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetFlagEffect(m)~=0 and Duel.IsChainNegatable(ev)
end

function cm.sptg1(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return Duel.IsExistingMatchingCard(aux.NegateMonsterFilter,tp,0,LOCATION_ONFIELD,1,nil) and c:IsAbleToRemove() end
	Duel.SetChainLimit(cm.chainlm)
	local g=Duel.GetMatchingGroup(aux.NegateMonsterFilter,tp,0,LOCATION_ONFIELD,nil)
...
function cm.aclimit(e,re,tp)
	return not re:GetHandler():IsType(TYPE_PENDULUM) and re:IsActiveType(TYPE_MONSTER)
end
