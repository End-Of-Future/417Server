--贝利尔 世荒的天灾[L]
function c88881799.initial_effect(c) 
	...
	e2:SetTarget(c88881799.reptg)
	c:RegisterEffect(e2)
end
function c88881799.zonelimit(e)
	...
	while tc do 
		...
		if Duel.GetLocationCount(tp,LOCATION_MZONE,tp,LOCATION_REASON_TOFIELD,(1<<(seq+1)))>0 then  
			if seq==1 then bit.bor(zone,1) end 
			if seq==2 then bit.bor(zone,2) end 
			if seq==3 then bit.bor(zone,4) end  
		end
	tc=g:GetNext()
	end 
	return zone
end
function c88881799.disval(e)
...
function c88881799.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
