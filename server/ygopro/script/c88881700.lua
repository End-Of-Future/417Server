...
function cm.tgfilter(c,e,tp)
    return c:IsSetCard(0x891) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and not c:IsCode(m)
end
...
function cm.tgop(e,tp,eg,ep,ev,re,r,rp)
    ...
    local tc=Duel.SelectMatchingCard(tp,cm.tgfilter,tp,LOCATION_DECK,0,1,1,nil,e,tp)
    if tc then
...
function cm.spop(e,tp,eg,ep,ev,re,r,rp)
    ...
    and tc:IsRelateToEffect(e) and Duel.Equip(tp,tc,c) then
        ...
        if Duel.IsPlayerCanDraw(tp,1) and Duel.SelectYesNo(tp,16*m) then
            Duel.BreakEffect()
