...
function cm.actcon(e,tp,eg,ep,ev,re,r,rp)
	return re:GetHandler()==e:GetHandler() and not (re==c15000280.self_effect1 or re:GetDescription()==aux.Stringid(m,0))
end
...
function cm.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)>=1 and Duel.SpecialSummonStep(c,0,tp,tp,false,false,POS_FACEUP) then
		...
		if not tc:IsType(TYPE_TRAPMONSTER) then
			local cid=c:CopyEffect(code,RESET_EVENT+RESETS_STANDARD+EVENT_PHASE+PHASE_END,1)
		end
		local e3=Effect.CreateEffect(c)
